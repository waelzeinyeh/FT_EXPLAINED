...
def del_freq():
    ...
    if session["msg_check"]=="Messages:On":
        session["message"]="""When hydrogen nuclei (protons) in a molecule are excited with a radiofrequency pulse, they will return to their steady state through a relaxation mechanism and emit a radiofrequency signal (the resonance signal).

<br><br>The frequency of each resonance depends on the chemical environment of the proton.<br><br>

In a 400MHz spectrometer, these resonances scale between 400MHz-5ppm and 400MHz+5ppm (approx.), which is between 399’998’000 Hz and 400’002’000 Hz.<br><br>

To improve clarity in this application, we will imagine that resonances scale from {} to {} Hz.<br><br>You can start adding frequencies with the button above.<br>""".format(session["freq_min"],session["freq_max"])
...
def draw_freq():
    ...
    resonance=np.zeros((3,session["nb_points"]))
    ...
    if max(FID)>-min(FID):
        ax_mult.set_ylim(-max(FID),max(FID))
        ax_FID.set_ylim(-max(FID),max(FID))
    else:
        ax_mult.set_ylim(min(FID),-min(FID))
        ax_FID.set_ylim(min(FID),-min(FID))
    if session["msg_check"]=="Messages:On":
        session["message"]="""More proton frequencies (up to 3) can be added using the same procedure.<br/><br/>
...
def new_freq():
    ...
    if session["freq_nb"]==3:
        session["FT_step"]=0
        t,resonance,FID,fig,ax_one,ax_FID,ax_mult,ax_FT=draw_freq()
        session["message"]="The app is limited to 3 frequencies in order to avoid crowding the display.<br>"
        return fig
    try:
...
def animate(f,t,resonance,FID,fig,ax_one,ax_FID,ax_mult,ax_FT,freqs,nb_freq,trial,mult,integ,posinteg,neginteg,n):
    ...
    for elem in reversed(ax_mult.collections+ax_mult.lines+ax_FID.lines+ax_FT.lines):
        elem.remove()
    ax_FID.plot(t,FID,linewidth=1,color="blue")
    ax_FID.plot(t,trial[f],linewidth=1,color="orange",alpha=0.5)
...
def step_1():
    ...
    ax_FID.plot(t,trial[n],linewidth=1,color="orange",alpha=0.5)
...
<img src="/static/img1.png" style="width:80%"/><br><br>
...
<img src="/static/img2.png" style="width:80%"/><br><br>
...
<img src="/static/img3.png" style="width:90%"/><br><br>
